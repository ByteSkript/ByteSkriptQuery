...
public class WriteLineEffect extends Effect {
    
    public WriteLineEffect() {
        super(ByteSkriptQuery.LIBRARY, StandardElements.EFFECT, "write line %Object%");
    ...
    public static void write(Object object) {
        ...
        if (!(thread.event instanceof RequestEvent event))
            throw new ScriptRuntimeError("The 'write' effect can only be used in the thread from a web request event.");
        event.request.response.append(System.lineSeparator()).append(object);
    }
    
    @Override
    public Pattern.Match match(String thing, Context context) {
        if (!thing.startsWith("write line ")) return null;
